...
describe('Controller: LoginController', function() {
    ...
    describe('initialisation', function() {
        ...
        it('should analyse the route parameters and automatically log a user in if they are set', function() {
            spyOn(apiService.session, 'getSession').and.returnValue({
                submit: function() {
                    return $q.when({
                        Session: 'asdhdashhds',
            ...
            expect(apiService.session.getSession).toHaveBeenCalledWith('asd9fugasd9ufgsaiduofb', 'sadf98hasf9dhsafushdao');
            expect(apiService.session.getSession.calls.count()).toBe(1);
    ...
    describe('login', function() {
        it('should call loginFailed if the getToken request fails', function() {
            spyOn(apiService.session, 'getToken').and.returnValue({
                submit: function() {
                    return $q.reject();
                }
            });
        ...
        it('should emit a loginSuccess event when the login request succeeds', function() {
            ...
            expect(apiService.session.getToken).toHaveBeenCalledWith();
            expect(apiService.session.getToken.calls.count()).toBe(1);
            expect(apiService.session.getSession).toHaveBeenCalledWith('myToksdd09hen', 'd77d1465d59a51b60d9ec1e79a58c921');
            expect(apiService.session.getSession.calls.count()).toBe(1);
